...
class RecentRecordsDetailsViewController: UIViewController {
    ...
    init(with viewModel: RecentRecordsDetailsViewModel) {
    ...
    func setupViews() {
        ...
        detailsTransaction.didTapOnSaveToMyAddress = {[weak self] type in
            ...
            case .save(let address):
                self.navigationController?.pushViewController(AddAddressViewController(with: AddAddressViewModel(with: CurrencyModel(coinType: 2, name: "ETH", description: "ETH"), and: address)), animated: true)
    ...
    func setData() {
        ...
        self.detailsTransaction.setData(with: transactionHistoryModel)
