...
class TimeGasTransactionDetailsView: UIView {
    ...
    private let confirmTimeView: TitleAndSubtitleView = {
        let view = TitleAndSubtitleView(with: TitleAndSubtitleModel(titleFont: UIFont.font(name: .semiBold, and: 16), titleTextColor: .wWLightBlack, subtitleFont: UIFont.font(name: .regular, and: 14), subtitleTextColor: .wBlack42, titleTextAlignment: .left, subTitleTextAlignment: .right))
        return view
    }()
    ...
    func setData(with transactionHistoryModel: TransactionHistoryModel) {
        ...
        networkFeeView.setData(with: "transaction_details_network_fee".localized, and: "\(transactionHistoryModel.fee.descriptionFor12Digits) ETH")
        gasPriceView.setData(with: "transaction_details_gas_price".localized, and: "\(transactionHistoryModel.gasPriceConv ?? "") ETH")
        gasLimitView.setData(with: "transaction_details_gas_limit".localized, and: "\(transactionHistoryModel.gasLimit ?? 0)")
        gasUsedView.setData(with: "transaction_details_gas_used".localized, and: "\(transactionHistoryModel.gasUsed ?? 0)")
    ...
    private func displayBottomSectionView(with shouldSeeMore: Bool) {
        ...
        seeMoreArrowImageView.image = UIImage(named: iconName)
    }
