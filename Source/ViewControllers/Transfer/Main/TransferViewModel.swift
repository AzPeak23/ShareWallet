...
class TransferViewModel {
    ...
    init(with currencyModel: CurrencyModel) {
        ...
        self.totalBalance = "\(fixedTotalBalance)"
    ...
    func updatedBalance(with type: TransferAmountView.ChangeCurrencyType) {
        ...
        case .all(let option):
            ...
            case .usdtTrc20, .usdtErc20:
                balanceToSend = "\(fixedTotalBalance)"
            default:
                balanceToSend = option == .toZero ? "" : "\(fixedTotalBalance - estimatedFees)"
            }
        ...
        case .normalCurrency(let balance):
            ...
            balanceToSend = ((Double(balance) ?? 0) * ratio).descriptionFor2Digits
        }
    ...
    func addNewtransfer(completionHandler: @escaping (Bool, String) -> Void) {
        ...
        let estimatedFeesDouble = gasFeesArry[gasFeesSelectedIndex].price

        SDKManager.shared.transfer(to: address, coinType: currencyModel.coinType,
            ...
            case .failure(let error):
                ...
                case .other(let meesage):
                    completionHandler(false, meesage.description)
                default:
